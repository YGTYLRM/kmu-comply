...
class TestProfiling:
    async def test_it_agency_enriched(self, profile_it_agency):
        result = await enrich_profile(profile_it_agency)
    ...
    async def test_healthcare_flags_special_data(self, profile_healthcare):
        result = await enrich_profile(profile_healthcare)
    ...
    async def test_manufacturer_flags_supply_chain(self, profile_manufacturer):
        result = await enrich_profile(profile_manufacturer)
        assert result.has_supply_chain_abroad is True

    async def test_missing_revenue_flagged(self, profile_it_agency):
        profile_it_agency.annual_revenue_eur = None
        result = await enrich_profile(profile_it_agency)
        assert "annual_revenue_eur" in result.missing_optional_fields

    async def test_fallback_on_no_api_key(self, profile_it_agency, monkeypatch):
...
class TestApplicability:
    async def test_freelancer_minimal(self, profile_freelancer):
        enriched = await enrich_profile(profile_freelancer)
    ...
    async def test_large_company_all_apply(self, profile_large_listed):
        enriched = await enrich_profile(profile_large_listed)
        result = determine_applicability(enriched)
    ...
    async def test_it_agency_gdpr_bdsg_only(self, profile_it_agency):
        enriched = await enrich_profile(profile_it_agency)
        result = determine_applicability(enriched)
        regs = {r.regulation for r in result if r.applies}
    ...
    async def test_applicability_has_reasons(self, profile_manufacturer):
        enriched = await enrich_profile(profile_manufacturer)
        result = determine_applicability(enriched)
...
class TestRetrieval:
    async def test_retrieves_gdpr_chunks(self, profile_it_agency):
        enriched = await enrich_profile(profile_it_agency)
        applicability = determine_applicability(enriched)
        chunks, _, _ = retrieve_regulatory_context(enriched, applicability)
        assert len(chunks) >= 5
        regs = {c.regulation for c in chunks}
        assert Regulation.GDPR in regs

    async def test_no_chunks_for_non_applicable(self, profile_freelancer):
        enriched = await enrich_profile(profile_freelancer)
        applicability = determine_applicability(enriched)
    ...
    async def test_manufacturer_retrieves_lksg(self, profile_manufacturer):
        enriched = await enrich_profile(profile_manufacturer)
        applicability = determine_applicability(enriched)
        chunks, _, _ = retrieve_regulatory_context(enriched, applicability)
        regs = {c.regulation for c in chunks}
    ...
    async def test_chunks_have_required_fields(self, profile_healthcare):
        enriched = await enrich_profile(profile_healthcare)
        applicability = determine_applicability(enriched)
        chunks, _, _ = retrieve_regulatory_context(enriched, applicability)
        for c in chunks:
...
class TestGapAndActions:
    async def test_gap_analysis_returns_results(self, profile_it_agency):
        from agent.planning import run_gap_analysis
        enriched = await enrich_profile(profile_it_agency)
        applicability = determine_applicability(enriched)
        chunks, empty_regs, _ = retrieve_regulatory_context(enriched, applicability)
        failures: list[str] = []
        gaps = await run_gap_analysis(enriched, chunks, failures, empty_regulations=empty_regs)
        assert isinstance(gaps, list)
        for gap in gaps:
            assert gap.evidence

    async def test_action_plan_covers_non_compliant(self, profile_it_agency):
