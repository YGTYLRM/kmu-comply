...
### JSON Fences from LLM

The model sometimes wraps JSON output in markdown code fences (` ```json ... ``` `). All LLM call sites use `_strip_fences()` before `json.loads()`. Any new LLM call that parses JSON **must** also call `_strip_fences()`.

### ChromaDB Singleton

`rag/ingest.py` and `rag/retrieval.py` each maintain a module-level `_chroma` singleton. Newly created collections within the same process are immediately visible. Cross-process visibility depends on ChromaDB's file persistence flush — a fresh process always reads the fully committed state.

### Backend Restart Required

No `--reload` flag. Code changes require killing and restarting the uvicorn process. The same applies to the worker.

### Stripe Tokens (in-memory)

Access tokens generated by Stripe checkout are stored in-memory in the backend process. They are lost on restart. This is acceptable for development; for production, move token storage to the PostgreSQL `subscriptions` table (already seeded in the DB schema).

### AI Act Phasing (legally uncertain)

`services/threshold_engine.py` and `rag/prompts.py` compute `date.today() >= date(2026, 8, 2)` at runtime. However, the Digital Omnibus proposal (provisional EU agreement, Reuters 2025) may delay Annex III high-risk obligations to 2 December 2027. Until formally adopted in the EU Official Journal, the system treats the effective date as uncertain and flags this in all AI Act findings — reporting PARTIALLY_COMPLIANT with an explanation of the date uncertainty rather than NON_COMPLIANT.
...
- Analysis: 10 screenings / hour / user (in-memory, resets on restart)
- Contact form: 5 requests / hour / IP (in-memory)

Both limits reset on backend restart. For production, move counters to Redis or the DB.
